...
\begin{figure}
    \includegraphics[width=\columnwidth]{media/evolution_nxm.pdf}

    \Description{Top: A monolithic DBMS with internal query engine, storage engine, and data layer. Bottom: Multiple query engines (DuckDB, Spark, Umbra) accessing a shared data lake containing files in various formats (Parquet, CSV, Vortex).}
...
This dynamic leads to format ossification, where datasets use outdated formats just to stay compatible, even when better encodings exist~\cite{kuiperQueryEnginesGatekeepers2025}. Changing formats is difficult because it risks breaking compatibility with many systems that use older specifications.

But what could solve this combinatorial integration burden? An abstraction layer between storage formats and processing systems reduces the $N \times M$ problem to $N + M$. Each system implements the interface once, and each format provides a single implementation against it. Gienieczko et al. (2025) propose exactly this and present AnyBlox as their realisation.