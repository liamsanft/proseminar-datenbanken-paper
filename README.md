# LaTeX template for Proseminar/Hauptseminar Datenbanken

This directory contains the main template to use for your paper in the Proseminar and Hauptseminar seminaries. It is based
on the official ACM LaTeX templates for conference proceedings. You should write your paper in the `main.tex` file. The
`samples` directory can be deleted. Notice however, the some of the example contents in the template reference files from this
directory. Therefore, the build will fail unless the references have been removed or replaced by your actual content. The same
applies to most of the other files (except for `acmart.cls`): they can be deleted as long as their functions are not used
(e.g. different citation styles).

**Be sure to read the source code as well as the rendered output of the `main.tex` file before inserting your own content.**
**It contains important information regarding the expected contents and formatting of your paper!.**

The full documentation of the ACM template can be found in `acmguide.pdf`. Notice however, that we apply some modifications
to the template and reduce the functionality slightly. In the end, our adapted version should be used.

## Original README of the ACM template
